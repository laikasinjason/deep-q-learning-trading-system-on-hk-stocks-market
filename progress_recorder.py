...
class ProgressRecorder():
    ...
    def process_recorded_data(self, **data):
        ...
        if self.__write_to_file:
            result = str(date.strftime("%Y-%m-%d")) + "," + str(bp) + "," + str(sp) + "," + str(profit) + "," + str(
                self.__cum_profit) + \
                     "," + str(self.__drawdown) + "," + str(pf_return) + "\n"

            self.write_to_file(result, "evaluation.txt")

    def write_after_evaluation_end(self, iteration, num_train):
        ...
        result = str(iteration) + "," + str(self.__cum_profit) + "," + str(self.__max_drawdown) + "," + str(
            sharpe_ratio) + "\n"

        self.write_to_file(result, "training_progress.txt")

        print("Iteration: " + str(iteration) + "/" + str(num_train) + ", "
              + "evaluation: max drawdown, cum profit " + str(self.__max_drawdown) + ", " + str(self.__cum_profit))
